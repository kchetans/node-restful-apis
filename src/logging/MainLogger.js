...
function GhostLogger(options) {
    ...
    if (!isLogFolderExist(this.path))
        createLogFolder(this.path)
...
function isLogFolderExist(path) {
    return fs.existsSync('tmp/logs')
}
...
GhostLogger.prototype.setFileStream = function () {
    this.streams['file-errors'] = {
        ...
        log: bunyan.createLogger({
            name: 'Log',
            streams: [{
                path: this.path + this.domain + '.error.log',
                level: 'error'
            }],
            serializers: this.serializers
        })
    ...
    this.streams['file-all'] = {
        ...
        log: bunyan.createLogger({
            name: 'Log',
            streams: [{
                path: this.path + this.domain + '.log',
                level: this.level
            }],
            serializers: this.serializers
        })
